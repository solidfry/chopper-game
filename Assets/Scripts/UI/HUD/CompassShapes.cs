...
namespace UI.HUD {
	...
	public class CompassShapes : MonoBehaviour {
		...
		public void DrawCompass( Vector3 worldDir ) {
			...
			Quaternion rotation = Quaternion.Euler( 0, 0, ShapesMath.TAU / Mathf.Rad2Deg );
			...
			Draw.Arc( compArcOrigin, bendRadius, lineThickness, angUiMin, angUiMax, ArcEndCap.Round, color );

			// draw the look angle label
			Draw.FontSize = fontSizeLookLabel;
			Draw.Text( labelPos, rotation, lookLabel, TextAlign.Center, font, color );

			// triangle arrow
			Vector2 trianglePos = compArcOrigin + Vector2.up * ( bendRadius + 0.01f );
			Draw.RegularPolygon( trianglePos, 3, triangleNootSize, -ShapesMath.TAU / 4, color );
			...
			void DrawTick( float worldAng, float size, string label = null ) {
				...
				Draw.Line( a, b, LineEndCap.None, color );
				if( label != null ) {
					Draw.FontSize = fontSizeTickLabel;
					Quaternion rotation = Quaternion.Euler( 0, 0, ( uiAng - ShapesMath.TAU / 4f ) * Mathf.Rad2Deg );
					Draw.Text( b - uiDir * tickLabelOffset, rotation, label, TextAlign.Center, font, color );
				}
			}
		}
